...
const TenantTransactions = () => {
  ...
  const stats = [
    { icon: TrendingUp,  label: 'Total Paid This Year', value: `₹${totalPaidThisYear.toLocaleString()}`, color: 'text-emerald-600', bgColor: 'bg-emerald-50' },
    { icon: Clock,       label: 'Pending Payments',     value: `₹${pendingPayments.toLocaleString()}`,   color: 'text-amber-600',   bgColor: 'bg-amber-50' },
    { icon: AlertCircle, label: 'Overdue',              value: `₹${overduePayments.toLocaleString()}`,   color: 'text-red-600',     bgColor: 'bg-red-50' },
    { icon: Calendar,    label: 'Upcoming (3mo)',        value: `₹${upcomingPayments.toLocaleString()}`,  color: 'text-blue-600',    bgColor: 'bg-blue-50' },
  ];
      ...
      <div className="flex h-screen bg-gray-50 overflow-hidden">
        ...
        >
          ...
          <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
            ...
            <SidebarItem icon={DollarSign} label="Transactions"  sidebarOpen active onClick={() => setSidebarOpen(false)} />
        ...
        >
          ...
          <nav className="flex-1 px-2 py-6 space-y-2 overflow-y-auto">
            ...
            <SidebarItem icon={DollarSign} label="Transactions"  sidebarOpen={sidebarOpen} active />
        ...
        <main className="flex-1 overflow-y-auto min-w-0">
          <div className="min-h-screen bg-white">
            ...
            <section className="relative bg-white pt-12 pb-0 overflow-hidden">
              <div className={`tx-container-custom relative z-10 ${!sidebarOpen ? 'md:pl-8' : ''}`}>
                <div className="pb-4">
                  <div className="tx-header-hero">
                    <div className="tx-header-title-row">
                      <div className="max-w-3xl flex-1">
                        <h1 className="tx-page-title mb-3">Track your<br /><span className="tx-title-accent">payment history</span></h1>
                        <p className="tx-header-subtitle">Stay on top of your rent payments. Manage all transactions, monitor payment status, and download receipts with ease.</p>
                      </div>
                      <button className="tx-export-btn flex-shrink-0"><Download /><span>Export</span></button>
                    </div>
                    ...
                    return (
                      <div key={idx} className="stat-card">
                        <div className="stat-card-header">
                          <div><p className="stat-label">{stat.label}</p><p className="stat-value">{stat.value}</p></div>
                          <div className={`stat-icon ${stat.bgColor}`}><Icon className={`w-5 h-5 ${stat.color}`} strokeWidth={1.5} /></div>
                        </div>
                      </div>
                    );
                  })}
            ...
            <section className="py-6 bg-white">
              <div className="tx-container-custom">
                <div className="tx-card-container">
                      ...
                      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                        <div className="flex flex-col gap-4">
                          <TabsList className="bg-gray-100 p-1 rounded-lg flex-shrink-0 w-full justify-start overflow-x-auto">
                            <TabsTrigger value="all"      className="px-3 py-1.5 text-xs font-medium">All</TabsTrigger>
                            <TabsTrigger value="past"     className="px-3 py-1.5 text-xs font-medium">Past</TabsTrigger>
                            <TabsTrigger value="current"  className="px-3 py-1.5 text-xs font-medium">Current</TabsTrigger>
                            <TabsTrigger value="upcoming" className="px-3 py-1.5 text-xs font-medium">Upcoming</TabsTrigger>
                          </TabsList>
                          <div className="filters-row">
                            <div className="relative flex-1">
                              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                              <Input placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 border-gray-200 focus-visible:ring-red-600 focus-visible:ring-offset-0 rounded-lg h-9 bg-gray-50 text-sm" />
                            </div>
                            <Select value={typeFilter} onValueChange={setTypeFilter}>
                              <SelectTrigger className="border-gray-200 focus:ring-red-600 rounded-lg bg-gray-50 h-9 w-full sm:w-40 text-sm"><SelectValue placeholder="Type" /></SelectTrigger>
                  ...
                  <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <TabsContent value="all"      className="mt-0"><TransactionTable transactions={allTabTransactions} /></TabsContent>
                    <TabsContent value="past"     className="mt-0"><TransactionTable transactions={pastTabTransactions} /></TabsContent>
                    <TabsContent value="current"  className="mt-0"><TransactionTable transactions={currentTabTransactions} /></TabsContent>
                    <TabsContent value="upcoming" className="mt-0"><TransactionTable transactions={upcomingTabTransactions} /></TabsContent>
                  </Tabs>
                </div>
                {allTabTransactions.length === 0 && (
                  <div className="mt-6 bg-white rounded-lg border border-gray-200 p-8 text-center">
                    <div className="w-14 h-14 bg-gray-100 rounded-xl flex items-center justify-center mx-auto mb-3"><Building2 className="w-7 h-7 text-gray-400" strokeWidth={1.5} /></div>
                    <h3 className="text-base font-semibold text-gray-900 mb-1">No Transactions Found</h3>
                    <p className="text-gray-600 max-w-md mx-auto text-sm">You don't have any transactions yet. Once you start renting, your payment history will appear here.</p>
                  </div>
                )}
              </div>
            </section>
                ...
                <div className="tx-container-custom">
                  <div className="cta-section">
                    <div className="cta-content">
                      <div className="cta-icon"><AlertCircle className="w-5 h-5 text-red-600" strokeWidth={2} /></div>
                      <div className="cta-text flex-1">
                        <h3>Action Required</h3>
                        <p>You have overdue payments totaling <span className="font-bold">₹{overduePayments.toLocaleString()}</span>. Please pay soon.</p>
