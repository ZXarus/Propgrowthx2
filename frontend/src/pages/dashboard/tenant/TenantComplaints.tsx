    ...
    <>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; font-family: 'Geist', sans-serif; }
        ...
        .page-title { font-family: 'Inter','Geist',system-ui,sans-serif; font-size: clamp(36px,4.5vw,56px); font-weight: 400; letter-spacing: -1.5px; line-height: 1.1; color: #0b1220; margin: 0; animation: slideInLeft 0.7s ease-out 0.1s both; }
        .title-accent { color: var(--brand-red); font-weight: 700; animation: slideInRight 0.7s ease-out 0.2s both; display: inline-block; }
        .container-custom { max-width: 1400px; margin: 0 auto; padding: 24px 32px; }
        @media (max-width: 767px) { .container-custom { padding: 56px 16px 16px; } }
        .header-hero { position: relative; padding: 32px 40px 36px; border-radius: 20px; background: linear-gradient(180deg,#FFF5F5 0%,#FFE4E6 100%); border: 1px solid rgba(220,38,38,0.12); animation: fadeInUp 0.8s ease-out 0s both; }
        .header-hero::after { content: ''; position: absolute; inset: 0; border-radius: 16px; pointer-events: none; box-shadow: 0 20px 50px rgba(2,6,23,0.05); }
        .header-title-row { display: flex; align-items: flex-start; gap: 14px; justify-content: space-between; flex-wrap: wrap; }
        .header-left { flex: 1; min-width: 300px; }
        .header-subtitle { font-size: 16px; color: var(--muted); font-weight: 400; letter-spacing: 0.2px; line-height: 1.6; margin-top: 12px; animation: fadeInUp 0.8s ease-out 0.25s both; }
        .divider-line { height: 1px; background: linear-gradient(90deg,rgba(220,38,38,0),rgba(220,38,38,0.3) 20%,rgba(220,38,38,0.5) 50%,rgba(220,38,38,0.3) 80%,rgba(220,38,38,0)); width: 100%; margin-top: 22px; animation: fadeInUp 0.8s ease-out 0.35s both; position: relative; }
        .back-btn { display: inline-flex; align-items: center; gap: 10px; background: var(--glass); border: 1px solid rgba(2,6,23,0.06); border-radius: 10px; padding: 10px 14px; cursor: pointer; transition: transform .16s ease,box-shadow .16s ease; backdrop-filter: blur(8px); font-weight: 600; color: #0b1220; animation: slideInLeft 0.7s ease-out 0s both; }
        ...
        .stats-grid { display: grid; grid-template-columns: repeat(1,minmax(0,1fr)); gap: 16px; }
        ...
        @media (max-width: 639px) { .stats-grid { grid-template-columns: repeat(2,1fr); } .stats-grid > :first-child { grid-column: span 2; } }
        .stat-card { position: relative; border-radius: 12px; overflow: hidden; padding: 16px; background: linear-gradient(180deg,#fff,#fbfcfd); border: 1px solid var(--card-border); box-shadow: 0 8px 24px rgba(2,6,23,0.03); transition: transform .18s cubic-bezier(.2,.9,.2,1),box-shadow .18s ease,border-color .18s ease; display: flex; flex-direction: column; gap: 10px; min-height: 120px; animation: fadeInUp 0.7s ease-out both; }
        .stat-card:nth-child(1) { animation-delay: 0.4s; } .stat-card:nth-child(2) { animation-delay: 0.5s; } .stat-card:nth-child(3) { animation-delay: 0.6s; } .stat-card:nth-child(4) { animation-delay: 0.7s; } .stat-card:nth-child(5) { animation-delay: 0.8s; }
        .stat-card::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 6px; background: linear-gradient(180deg,rgba(220,38,38,0.12),rgba(220,38,38,0.06)); opacity: 0.95; }
        .stat-top { display: flex; gap: 12px; align-items: center; }
        .stat-icon { width: 48px; height: 48px; border-radius: 10px; background: linear-gradient(180deg,#fff,#f7f8fb); display: inline-flex; align-items: center; justify-content: center; border: 1px solid rgba(2,6,23,0.03); box-shadow: 0 6px 18px rgba(2,6,23,0.03); flex-shrink: 0; animation: scaleIn 0.6s cubic-bezier(.2,.9,.2,1) both; }
        .stat-card:nth-child(1) .stat-icon { animation-delay: 0.5s; } .stat-card:nth-child(2) .stat-icon { animation-delay: 0.6s; } .stat-card:nth-child(3) .stat-icon { animation-delay: 0.7s; } .stat-card:nth-child(4) .stat-icon { animation-delay: 0.8s; } .stat-card:nth-child(5) .stat-icon { animation-delay: 0.9s; }
        ...
        .stat-card.urgent { background: linear-gradient(180deg,rgba(220,38,38,0.06) 0%,#fff9f9 100%); border: 1.2px solid rgba(220,38,38,0.18); box-shadow: 0 8px 24px rgba(220,38,38,0.09); }
        ...
        .resolution-card { margin-top: 18px; border-radius: 12px; padding: 16px; display: flex; gap: 16px; align-items: center; justify-content: space-between; border: 1px solid rgba(2,6,23,0.04); background: linear-gradient(180deg,#fff,#fcfcfc); box-shadow: 0 14px 40px rgba(2,6,23,0.03); animation: fadeInUp 0.8s ease-out 0.9s both; }
        ...
        .progress-circle { width: 84px; height: 84px; border-radius: 50%; display: flex; align-items: center; justify-content: center; background: conic-gradient(var(--brand-red) 0deg,#e6eef6 0deg); position: relative; box-shadow: 0 8px 20px rgba(2,6,23,0.04); flex-shrink: 0; animation: scaleIn 0.7s cubic-bezier(.2,.9,.2,1) 0.95s both; }
        .progress-circle .inner { width: 64px; height: 64px; border-radius: 50%; background: #fff; display: flex; align-items: center; justify-content: center; font-weight: 700; color: #0b1220; box-shadow: 0 2px 8px rgba(2,6,23,0.04); }
        .complaint-section { margin-top: 18px; animation: fadeInUp 0.8s ease-out 1s both; }
        @media (max-width: 768px) { .header-title-row { flex-direction: column; gap: 20px; } .page-title { font-size: 28px; } }
        @media (max-width: 640px) { .header-hero { padding: 16px 18px 20px; border-radius: 12px; } .back-btn { padding: 8px 10px; font-size: 12px; gap: 6px; } .page-title { font-size: 24px; } .header-subtitle { font-size: 13px; margin-top: 8px; } .divider-line { margin-top: 14px; } }
      `}</style>
      ...
      <div className="flex h-screen bg-gray-50 overflow-hidden">
        ...
        <button
          className="md:hidden fixed top-3 left-3 z-50 w-10 h-10 flex items-center justify-center
            bg-white border border-gray-200 rounded-xl shadow-md transition-all duration-200 hover:bg-gray-50"
        ...
        >
          ...
          <div className="px-2 py-4 border-t border-gray-200 space-y-1">
            ...
            <SidebarItem icon={Settings}   label="Settings" sidebarOpen onClick={() => mobileGoTo('/profile#settings')} />
        ...
        <main className="flex-1 overflow-y-auto min-w-0">
          <div className="min-h-screen bg-white">
            <div className="container-custom">
                ...
                <div className="header-hero">
                  <div className="header-title-row">
                    <div className="header-left">
                      <h1 className="page-title mb-3">Report issues,<br /><span className="title-accent">get resolutions</span></h1>
                      <p className="header-subtitle">Submit maintenance requests and complaints. Track resolution status in real-time and communicate with property management.</p>
                    </div>
                    <Button onClick={() => setIsAddModalOpen(true)} className="bg-[#DC2626] hover:bg-red-700 text-white font-semibold px-6 py-2.5 rounded-lg new-complaint-btn h-fit whitespace-nowrap">
              ...
              <section className="py-6">
                <div className="stats-grid">
                  <div className="stat-card urgent">
                    <div className="stat-top"><div className="stat-icon" aria-hidden><TrendingUp /></div><div><div className="stat-number">{stats.urgent}</div><div className="stat-label">Urgent</div></div></div>
                  </div>
                  <div className="stat-card">
                    <div className="stat-top"><div className="stat-icon" aria-hidden><BarChart3 /></div><div><div className="stat-number">{stats.total}</div><div className="stat-label">Total</div></div></div>
                  </div>
                  <div className="stat-card open">
                    <div className="stat-top"><div className="stat-icon" aria-hidden><AlertCircle /></div><div><div className="stat-number">{stats.open}</div><div className="stat-label">Open</div></div></div>
                  </div>
                  <div className="stat-card progress">
                    <div className="stat-top"><div className="stat-icon" aria-hidden><Clock /></div><div><div className="stat-number">{stats.inProgress}</div><div className="stat-label">In Progress</div></div></div>
                  </div>
                  <div className="stat-card resolved">
                    <div className="stat-top"><div className="stat-icon" aria-hidden><CheckCircle /></div><div><div className="stat-number">{stats.resolved}</div><div className="stat-label">Resolved</div></div></div>
                  </div>
                </div>
                ...
                {stats.total > 0 && (
                  <div className="resolution-card">
                    <div className="resolution-left">
                      ...
                      <div className="progress-bar" aria-hidden><div className="progress-fill" style={{ width: `${resolutionRate}%` }} /></div>
                    </div>
                    <div className="progress-circle" aria-hidden style={{ background: `conic-gradient(var(--brand-red) 0deg ${resolutionRate * 3.6}deg, #eef2f7 ${resolutionRate * 3.6}deg 360deg)` }}>
              ...
              <AddComplaintModal open={isAddModalOpen} onOpenChange={setIsAddModalOpen} />

              <div className="complaint-section pb-8">
                <ComplaintList
                  complaints={tenantComplaints}
                  setComplaints={setComplaints}
                  onSelect={(complaint) => { setSelectedComplaint(complaint); setIsDetailModalOpen(true); }}
...
const SidebarItem = ({
  icon: Icon, label, active = false, onClick, sidebarOpen = true,
}: {
  icon: React.ElementType; label: string; active?: boolean; onClick?: () => void; sidebarOpen?: boolean;
}) => (
  <button
    onClick={onClick}
